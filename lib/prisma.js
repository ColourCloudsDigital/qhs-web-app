  ...
  booking: {
    findUnique: async ({ where, include }) => {
      ...
      if (include) {
        ...
        if (include.room) {
          const rooms = await prisma.query('SELECT * FROM rooms WHERE id = ? LIMIT 1', [booking.roomId]);
          ...
          if (include.hotel || include.room?.include?.hotel) {
            ...
            if (room) {
              const hotels = await prisma.query('SELECT * FROM hotels WHERE id = ? LIMIT 1', [room.hotelId]);
              room.hotel = hotels[0] || null;
            }
          }
        }
    ...
    findMany: async ({ where, orderBy, take, skip, include }) => {
      ...
      if (include && bookings.length > 0) {
        ...
        if (include.room) {
          ...
          bookings.forEach(booking => {
            booking.room = roomMap[booking.roomId] || null;
          });
        }
