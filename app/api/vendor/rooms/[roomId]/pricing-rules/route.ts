...
export async function PUT(
  ...
  try {
    ...
    // Validate required fields
    if (!ruleId || !startDate || !endDate || priceAdjustment === undefined || !adjustmentType) {
      return NextResponse.json(
        { error: 'Missing required fields' },
        { status: 400 }
      );
    ...
    // Check if rule exists and belongs to this room
    ...
    // Check for overlapping date ranges (excluding this rule)
    const [overlapRows]: any = await pool.query(
      `SELECT id 
       FROM room_pricing_rules 
       WHERE roomId = ? 
