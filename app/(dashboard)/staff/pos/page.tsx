...
export default function StaffPOSPage() {
  ...
  const fetchProcessed = () => {
    ...
    const p = new URLSearchParams({ page: String(ordersPage), limit: String(ordersLimit), hotelId: currentHotel.id, excludeStatus: 'placed' });
  ...
  const handlePlaceOrder = async () => {
    ...
    const payload = isEdit
      ? { orderId: activeOrder.id, status: 'placed', paymentStatus: activeOrder.paymentStatus, paymentMethod: activeOrder.paymentMethod, totalAmount, items: activeOrder.items.map(i => ({ id: i.id, quantity: i.quantity, price: i.price })) }
      : { vat: activeOrder.vat, totalAmount, items: activeOrder.items.map(i => ({ id: i.id, quantity: i.quantity, price: i.price })), paymentMethod: activeOrder.paymentMethod, paymentStatus: activeOrder.paymentStatus, hotelId: currentHotel?.id, vendorId: session?.user?.vendorId };
    ...
    try {
      ...
      if (res.ok && data.success) {
        clearActiveOrder();
        toast.success(isEdit ? 'Order updated!' : 'Order placed!');
        fetchPlaced();
  ...
  return (
    <div className="flex flex-col gap-4 p-4 md:flex-row">
      <div className="flex flex-1 flex-col gap-4">
        <OrderLine selectedTab={selectedTab} setSelectedTab={setSelectedTab} placedOrdersCount={placedTotal} processedOrdersCount={ordersTotal} />

        {selectedTab === 'current' && (
          loadingMenus ? <div className="flex items-center justify-center py-16 text-gray-400 text-sm">Loading menu...</div>
          : menus?.categories?.length ? <FoodMenu categories={menus.categories} onAddItem={addItemToActiveOrder} />
          : <div className="flex flex-col items-center justify-center py-16 text-gray-400"><p className="text-sm">No menu items available.</p></div>
        )}

        {selectedTab === 'placed' && (
          <PlacedOrdersTable orders={placedOrders} page={placedPage} total={placedTotal} limit={placedLimit} isLoading={placedLoading} onPageChange={setPlacedPage} onLimitChange={setPlacedLimit} onRefresh={fetchPlaced} onEditOrder={handleEditPlacedOrder} />
        )}
