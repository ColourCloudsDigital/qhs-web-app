...
interface RoomImagesUploaderProps {
...
export function RoomImagesUploader({
  existingImages,
  hotelId,
  roomId,
  onImagesChange,
  maxImages = 6
}: RoomImagesUploaderProps) {
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const entityPath = roomId 
        ? `hotels/${hotelId}/rooms/${roomId}`
        : `hotels/${hotelId}/rooms/temp-${Date.now()}`;
          
      // Build the query params
      const queryParams = new URLSearchParams();
      queryParams.append('entity', entityPath);
  ...
  return (
    <FormSection title="Room Images">
      ...
      <FormField 
        label="Upload Images" 
        helperText={`Upload up to ${maxImages} images. Images should showcase the room interior, bathroom, and views. Supported formats: JPG, PNG, WebP. Max size: 2MB per image.`}
      >
        <div>
          <label 
